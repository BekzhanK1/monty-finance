...
from app.models.models import User, Category, Transaction, TransactionType
...
def get_analytics(
    ...
    total_expenses = sum(t.amount for t in transactions if t.category.type == TransactionType.EXPENSE)
    ...
    for t in transactions:
        ...
        if cat_name not in by_category:
            by_category[cat_name] = {"name": cat_name, "icon": cat_icon, "income": 0, "expense": 0}
    ...
    by_category_list = [{"name": v["name"], "icon": v["icon"], "amount": v["expense"] if v["expense"] > 0 else v["income"]} for v in by_category.values()]
